...
            var X = event.touches[0].clientX;
	...
	return(
		...
		className={`overflow-hidden md:w-[22%] w-0 ${revealMenu ? "w-[80%]  transform transition-width duration-500 ease-in-out" : "w-[0%] transform transition-width duration-500 ease-in-out" } h-screen relative bg-[#120F13] relative`}>
			<header className="w-full flex  p-4 shadow-md shadow-black/50 justify-between items-center" >	
				{
					...
					<div className="flex md:gap-5 gap-3 w-full items-center" >	
						...
						className="h-8 w-8 text-gray-500/90  hover:bg-gray-800/40 rounded-lg 
			...
			<div className={`absolute ${revealState ? "scale-100  z-50 transition-bottom duration-500 ease-in-out opacity-100" : "scale-0 transition-bottom z-0 duration-500 ease-in-out opacity-0 "} 
			 transition duration-500 right-5 bottom-20 ease-in-out p-2 flex flex-col border 
			bg-gray-800/80 border-gray-600 rounded-xl w-30`} >
				<div 
